...
function setup() {
  size = 400;
  cnv = createCanvas(size, size).parent('canvas-holder');
  buttonShow = true;
  playAnimation = false;
}
...
function draw() {
  ...
  if (buttonShow === true) {
    ...
    strokeJoin(ROUND);
    triangle(
      buttonPosx + 0.43 * width,
      buttonPosy + 0.38 * height,
      buttonPosx + 0.43 * width,
      buttonPosy + 0.62 * height,
      buttonPosx + 0.57 * width,
      buttonPosy + 0.5 * height
    );
  }
