...
function setup() {
  ...
  if (getMobile() === false) {
    ...
    createCanvas(screen.width - 40, h1 / 3).parent('canvas-holder');
  }
  setWindow(-5, 3, -3, 3);
  background(bgcolor);
...
function draw() {
  ...
  let p1 = worldToScreen(x0, f(x0)),
    p2 = worldToScreen(x0 + h, f(x0 + h));
  ...
  const p3 = worldToScreen(xmin, f1(xmin)),
    p4 = worldToScreen(xmax, f1(xmax));
  stroke(255);
